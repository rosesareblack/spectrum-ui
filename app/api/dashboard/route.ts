...
export async function PATCH(req: NextRequest, res: NextResponse) {
  ...
  try {
    ...
    const approveComp = await Component.findByIdAndUpdate(reqBody.id, { status: reqBody.status }, { new: true });

    if (!approveComp) {
      return NextResponse.json({ message: "Component not found" }, { status: 404 });
    }

    return NextResponse.json({ message: "Status updated", approveComp }, { status: 200 });
  } catch (error: any) {
